...
Syedra core also provides an asynchronous version of the
Block architecture that is compatible with asyncio
framework.

*NOTE:* Currently asynchronous and synchronous uses of the
Block architecture cannot be mixed together. The user must
select one or the other. 

The utilization of this base class is mostly identical to
that of the Block with only three main differences:

  * Importing Asynchronous Block
  * Asynchronous Block class implementation
  * Execution of Asynchronous Block pipelines

The AsyncBlock class is imported from the same module.
  
#+begin_src python
from syedra.code.block import AsyncBlock
#+end_src

Derived asynchrouns block classes define the *update()*
methods as a coroutine.

#+begin_src python
class MyBlock(AsyncBlock):
  block_name = 'my_block'
...
The execution of the asynchronous block pipeline must be
done by calling the *AsyncBlock.execute* using *await*.
